...
procedure plotCruveImp;
    var menorX, menorY, maiorX, maiorY, passo : double;
    var i, j : double;
    var valorFuncao, valorFuncaoTriangulacaoDesc, valorFuncaoTriangulacaoAsc : double;
    var pontoInterno, pontoInternoTriangulacaoDesc, pontoInternoTriangulacaoAsc : boolean;
    var larguraCaneta, meiaLargura : integer;
begin
    //  Ajusta a cor da pintura
    JPEGExampleForm.Image1.Canvas.Brush.Color:=RGBToColor(0,0,255);

    // Largura da caneta
    larguraCaneta := 4;
    meiaLargura := (larguraCaneta div 2);
    ...
    // Itera entre os limites do gráfico, na resolução indicada
    i := menorX;
    while (i <= maiorX) do
    begin
        j := menorY;
        while (j <= maiorY) do
        begin
            // Calcula valor da função no ponto atual e em um ponto
            // acima e à esquerda
            valorFuncao := f(i, j);
            valorFuncaoTriangulacaoDesc := f(i-passo, j-passo);
            valorFuncaoTriangulacaoAsc := f(i+passo, j-passo);

            // O ponto é interno à figura se o seu valor for negativo
            // O ponto é externo à figura se o seu valor for positivo
            // O ponto está no contorno da figura se o seu valor for negativo
            pontoInterno := (valorFuncao < 0);
            pontoInternoTriangulacaoDesc := (valorFuncaoTriangulacaoDesc < 0);
            pontoInternoTriangulacaoAsc := (valorFuncaoTriangulacaoAsc < 0);

            // Se o ponto está no contorno ou
            // se mudou o sinal, o contorno está nesta faixa
            if (pontoInterno <> pontoInternoTriangulacaoDesc)
                or (pontoInterno <> pontoInternoTriangulacaoAsc)
                or (valorFuncao = 0) then
            begin

                // Preenche um retângulo da largura da caneta
                // ao redor do ponto atual
                JPEGExampleForm.Image1.Canvas.FillRect(
                    xToInteger(i)-meiaLargura,
                    yToInteger(j)-meiaLargura,
                    xToInteger(i)+meiaLargura,
                    yToInteger(j)+meiaLargura
                );
            end;
